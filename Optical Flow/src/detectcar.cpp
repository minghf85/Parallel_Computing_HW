...
struct Vehicle {
    ...
    Vehicle(int _id, cv::Point2f _center) : 
        id(_id), center(_center), counted(false), framesExisted(1) {
...
int main() {
    // 打开视频文件
    cv::VideoCapture cap("H:/Project/Parallel_Computing_HW/Optical Flow/data/testspeedup.mp4");
    if (!cap.isOpened()) {
    ...
    writer.open("h:\\Project\\Parallel_Computing_HW\\Optical Flow\\detection_result.mp4",
    ...
    while (true) {
        ...
        for (auto it = vehicles.begin(); it != vehicles.end();) {
            ...
            for (size_t i = 0; i < vehicle_centers.size(); i++) {
                if (detected_vehicles[i]) continue;
                
                float distance = cv::norm(it->center - vehicle_centers[i]);
                if (distance < 50) { // 距离阈值，可根据实际情况调整
                    // 更新车辆位置
            ...
            if (!found_match) {
                if (it->framesExisted > 5) { // 至少跟踪5帧以避免误检
    ...
    std::cout << "finished total" << vehicle_counter << " cars." << std::endl;
